...
import { getProducts, unSetCurrent } from "../../../slice/product.slice.ts";
  ...
  useEffect(() => {
    if (current) {
      setProduct({
        ...
        year: current?.year,
        techSpecification: current?.techSpecification,
      });
      setValue(current?.information);
    }
  }, [current]);
  const handleUpdateProductBtn = async () => {
    if (
      ...
      try {
        ...
        await axios.patch("/api/admin/product/" + current?._id, data);
    ...
    <div>
      ...
      <div className="modal">
        ...
        <div className="modal-content flex flex-col gap-5 min-w-full">
          ...
          <h2 className="text-xl">Sửa sản phẩm</h2>
          <div className="modal-contents flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <h1 className="font-medium text-error">Ảnh mới</h1>
              <input
                type="file"
                multiple={true}
                ref={fileInputRef}
            ...
            <div className="flex flex-col gap-2">
              <h1 className="font-medium text-error">Ảnh cũ</h1>
              <div className="grid grid-cols-4 gap-3">
                {current?.imageList &&
